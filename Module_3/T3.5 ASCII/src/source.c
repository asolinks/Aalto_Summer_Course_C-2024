...
{    int count = 0;
    ...
    for (int i = min; i <= max; ++i) {
        // Integer value (right aligned, at least 3 characters)
        printf("%3d ", i);
        
        // Hexadecimal value (0xXX, 4 characters wide)
        printf("0x%02x ", i);
        
        // ASCII character (printable check)
        if (isprint(i)) {
            printf("%c", i);
        } else {
            printf("?");
        ...
        // Check if it's the last column of the row
        if (++count % 4 == 0) {
            printf("\n"); // End of row
        } else {
